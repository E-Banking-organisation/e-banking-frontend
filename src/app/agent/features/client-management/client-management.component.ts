...
export class ClientManagementComponent implements OnInit {
  ...
  saveClient(): void {
    ...
    if (this.isEditMode) {
      ...
      this.clientService.updateClient(clientData).subscribe(
        updatedClient => {
          if (updatedClient) {
            this.clients = this.clients.map(c => c.id === clientData.clientId ? updatedClient : c);
            this.applyFilters();
            this.showClientModal = false;
          }
      ...
      const mdp = this.clientForm.value.password;
      this.clientService.createClient(clientData, mdp).subscribe(
        clientId => {
          const newClient: Client = { ...clientData, id: clientId }; // clone et ajoute l'id
  ...
  activateClient(clientData: Client): void {
    if (confirm('Êtes-vous sûr de vouloir activer ce client?')) {
      this.updateClientStatus(clientData, 'active');
    }
  }

  suspendClient(clientData: Client): void {
    if (confirm('Êtes-vous sûr de vouloir suspendre ce client?')) {
      this.updateClientStatus(clientData, 'suspended');
    }
  }
  ...
  private updateClientStatus(clientData: Client, status: 'active' | 'pending' | 'suspended' | 'closed'): void {
    this.clientService.updateClient(clientData).subscribe(
      updatedClient => {
        if (updatedClient) {
          this.clients = this.clients.map(c => c.id === clientData.id ? updatedClient : c);
          this.applyFilters();

          if (this.selectedClient && this.selectedClient.id === clientData.id) {
